      ...
      <form id="form-data">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              <label>Status Listrik</label><br>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="status_listrik" id="status_listrik1" value="1">
                <label class="form-check-label" for="status_listrik1">Aktif</label>
              </div>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="status_listrik" id="status_listrik0" value="0">
                <label class="form-check-label" for="status_listrik0">Nonaktif</label>
              </div>
            </div>
          </div>
    ...
    <script>
        ...
        function detailData(id) {
            var selectedData = tableData.find(function(item) {
                return item.id_ref_lokasi == id;
            });

            console.log(id);

            if (selectedData) {
                urlAction = baseUrl + '/update/' + selectedData.id_ref_lokasi;
                actionMethod = 'PUT';
                ...
                $('#id').val(selectedData.id_ref_lokasi);
                $('#nama_lokasi').val(selectedData.nama_lokasi);
                $('#alamat').val(selectedData.alamat);
                $('#lat').val(selectedData.lat);
                $('#lon').val(selectedData.lon);

                if (selectedData.status_listrik == 1) {
                    $('#status_listrik1').prop('checked', true);
                } else {
                    $('#status_listrik0').prop('checked', true);
                }

                $('#modal-data').modal('show');
            } else {
                console.error('Lokasi item not found');
            }
        }
        ...
        loadData = function() {
            ...
            $("#jqxGrid").jqxGrid({
                ...
                columns: [
                    { text: 'Nama Lokasi', datafield: 'nama_lokasi' },
                    { text: 'Alamat', datafield: 'alamat' },
                    { text: 'Latitude', datafield: 'lat' },
                    { text: 'Longitude', datafield: 'lon' },
                    { 
                        text: 'Status', 
                        datafield: 'status_listrik', 
                        width: '5%',
                        cellsalign: 'center',
        ...
        $(document).ready(function() {
            ...
            $('#form-data').validate({
                rules: {
                    nama_lokasi: { required: true, minlength: 3 },
                    alamat: { required: true },
                    status_listrik: { required: true }
                },
                messages: {
                    nama_lokasi: { required: "Kolom Nama Lokasi wajib diisi.", minlength: "Nama Lokasi harus terdiri dari minimal 3 karakter." },
                    alamat: { required: "Kolom Alamat wajib diisi." },
                    lat: { required: "Kolom Latitude wajib diisi." },
                    lon: { required: "Kolom Longitude wajib diisi." },
                    status_listrik: { required: "Kolom Status Listrik wajib diisi." }
                },
