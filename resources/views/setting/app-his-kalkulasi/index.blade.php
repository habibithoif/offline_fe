        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
                <label for="datum">Tanggal</label>
                <input type="datetime-local" class="form-control" name="datum" id="datum" required>
            </div>
            ...
            <div class="form-group">
              ...
              <select name="jenis" id="jenis" class="form-control select2" required>
                ...
                <option value="FASOP_WA">FASOP_WA</option>
                <option value="KIN_ANALOG_HARIAN">KIN_ANALOG_HARIAN</option>
                <option value="KIN_DIGITAL_HARIAN">KIN_DIGITAL_HARIAN</option>
              </select>
            </div>

            {{-- <div class="form-group">
              <label for="id_pemilik">Lokasi</label>
            ...
            <div class="form-group">
              ...
              <div class="form-check form-check-inline">
                ...
                <label class="form-check-label" for="status1">Hitung Ulang</label>
              </div>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="status" id="status0" value="2">
                <label class="form-check-label" for="status0">Selesai dihitung</label>
              </div>
            </div>
          </div>
    ...
    <script>
        ...
        function detailData(id) {
            ...
            if (selectedData) {
                ...
                var datum = new Date(selectedData.datum);  // Ensure it's a Date object
                var formattedDate = datum.toISOString().slice(0, 16);  // Format as YYYY-MM-DDTHH:MM
                $('#datum').val(formattedDate);
                $('#name').val(selectedData.name);
                $('#jenis').val(selectedData.jenis).trigger('change');
        ...
        loadData = function() {
            ...
            var source = {
                ...
                datafields: [
                    ...
                    { name: 'mulai_kalkulasi', type: 'datetime' },
                    { name: 'selesai_kalkulasi', type: 'datetime' },
                    { name: 'status', type: 'string' },
                ],
                url: '{{ route("app-his-kalkulasi.read") }}',
                cache: false,
                root: 'Rows',
            ...
            $("#jqxGrid").jqxGrid({
                ...
                columns: [
                    { text: 'Tanggal Kalkulasi', datafield: 'datum' },
                    { text: 'Nama Kalkulasi', datafield: 'name', width: '20%' },
                    { text: 'Jenis', datafield: 'jenis', width: '20%' },
                    { text: 'Tanggal Mulai', datafield: 'mulai_kalkulasi', width: '15%' },
                    { text: 'Tanggal Selesai', datafield: 'selesai_kalkulasi', width: '10%' },
                    { 
                        text: 'Status', 
                        datafield: 'status', 
                        cellsalign: 'center',
                        cellsrenderer: function(row, column, value, defaultHtml, columnSettings, rowData) {
                            var statusHtml = '';
                            if (value == 2 && rowData.selesai_kalkulasi ) {
                                statusHtml = '<span class="badge badge-success">Selesai Hitung</span>';
                            } else {
                                statusHtml = '<span class="badge badge-danger">Unknown</span>';
                            }
        ...
        $(document).ready(function() {
            ...
            $('#form-data').validate({
                ...
                messages: {
                    ...
                    jenis: { required: "Kolom Display wajib diisi." },
