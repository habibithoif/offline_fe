...
class MasterDataRTUController extends Controller
{
    ...
    public function index()
    {
        ...
        $ref_region_filter = $this->apiRequest('get', 'cpoint', ['point_type' => 'RTU']);
        $this->data->ref_region_filter = $ref_region_filter['data']['Rows'] ?? [];
        $this->data->pointtype_name = 'RTU';
        $data['id_induk_pointtype'] = '3d3918194288469980f47ebd5ae0d733';
        $point_type = $this->apiRequest('get', 'pointtype/find-child', $data);
        $this->data->point_type = $point_type['data'] ?? [];
        return view('MasterData.fasop.rtu.index', ['data' => $this->data]);
    }

    public function read(Request $request)
    {
        $payload = $request->all();
        $payload['point_type'] = 'RTU';
